...
public class SaveSystemStorage : ScriptableObject
{
...
#if UNITY_EDITOR
    [MenuItem("Framework/StoreObjects")]
    public static void StoreObjects()
    {
        SaveSystemStorage database = GetAllAssetsAtPath<SaveSystemStorage>("", true)[0];
        if (database == null)
        {
            Debug.LogError("Create a database scriptable");
            return;
        }
